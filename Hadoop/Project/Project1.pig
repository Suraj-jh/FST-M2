...
inputDialogues4 = LOAD '/user/surajha/inputs/episodeIV_dialogues.txt' using PigStorage('\t') AS (name:chararray,line:chararray);
inputDialogues5 = LOAD '/user/surajha/inputs/episodeV_dialogues.txt' using PigStorage('\t')  AS (name:chararray,line:chararray);
inputDialogues6 = LOAD '/user/surajha/inputs/episodeVI_dialogues.txt' using PigStorage('\t')  AS (name:chararray,line:chararray);
...
rmf hdfs:///user/surajha/Outputs;

-- Store the result in HDFS
STORE namesOrdered INTO 'hdfs:///user/surajha/Outputs' using PigStorage('\t');